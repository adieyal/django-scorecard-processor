...
from guardian.shortcuts import get_users_with_perms, assign, remove_perm
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for entity in orm.Entity.objects.all():
            for user in entity.user_set.all():
                assign('change_entity', User.objects.get(pk=user.pk), entity)
                entity.user_set.remove(user)


    def backwards(self, orm):
        for entity in orm.Entity.objects.all():
            for user in get_users_with_perms(entity, with_group_users=False):
                remove_perm('change_entity', user, entity)
                entity.user_set.add(orm['auth.User'].objects.get(pk=user.pk))
    ...
    complete_apps = ['guardian', 'scorecard_processor']
