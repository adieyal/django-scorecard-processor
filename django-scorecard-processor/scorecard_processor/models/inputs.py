...
class ExcelFile(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if new:
            call_command('1_1_birth_registration', self.excel_file.path, survey_name='BirthRegistration', excel_file_id=self.id)
            call_command('1_2_death_registration', self.excel_file.path, survey_name='DeathRegistration', excel_file_id=self.id)
            call_command('1_3_maternal_death_reviews', self.excel_file.path, survey_name='MaternalDeathReviews', excel_file_id=self.id)
            call_command('1_4_cod', self.excel_file.path, survey_name='COD', excel_file_id=self.id)
            call_command('1_5_qoc', self.excel_file.path, survey_name='QoC', excel_file_id=self.id)
            call_command('1_7_crvs_improvement', self.excel_file.path, survey_name='CRVSImprovement', excel_file_id=self.id)
            call_command('2_1_health_info', self.excel_file.path, survey_name='HealthInfo', excel_file_id=self.id)
            call_command('2_2_coverage_indicators', self.excel_file.path, survey_name='CoverageIndicators', excel_file_id=self.id)
            call_command('2_3_impact_indicators', self.excel_file.path, survey_name='ImpactIndicators', excel_file_id=self.id)
            call_command('3_1_ehealth_strategy', self.excel_file.path, survey_name='EhealthStrategy', excel_file_id=self.id)
            call_command('3_2_web_based_reporting', self.excel_file.path, survey_name='WebBasedReporting', excel_file_id=self.id)
            call_command('4_1_total_health_expenditure', self.excel_file.path, survey_name='TotalHealthExpenditure', excel_file_id=self.id)
            call_command('4_2_nha_conducted', self.excel_file.path, survey_name='NhaConducted', excel_file_id=self.id)
            call_command('4_3_rmnch_expenditure', self.excel_file.path, survey_name='RmnchExpenditure', excel_file_id=self.id)
            call_command('5_1_financial_reporting_system', self.excel_file.path, survey_name='FinancialReportingSystem', excel_file_id=self.id)
            call_command('6_1_reviews_of_health_spending', self.excel_file.path, survey_name='ReviewsOfHealthSpending', excel_file_id=self.id)
            call_command('7_1_reviews_of_performance', self.excel_file.path, survey_name='ReviewsOfPerformance', excel_file_id=self.id)
            call_command('8_1_performance_report_public', self.excel_file.path, survey_name='PerformanceReportPublic', excel_file_id=self.id)
